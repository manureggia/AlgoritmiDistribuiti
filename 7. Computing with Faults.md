...
Dimostriamo ora le varie proprietà che deve rispettare questo algoritmo

**Non triviality**

<img src="Immagini/Faults/non-triv-1.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

Hanno un numero sufficiente di proposte `1,v` per poter decidere per `v`

**Agreement**

Se tutti partono con lo stesso valore abbiamo appena visto che funziona. ma cosa succede nel caso non tutti partono con lo stesso valore?

se c'è una non faulty che decide per uno dei due valori allora anche le altre non faulty decideranno per lo stesso valore.

c'è una entità `x` che ha deciso per un v, vuol dire che ha visto almeno $f+1$ `propose(r,v)` (ricordiamo che $f$ sono il numero di entità che possono crashare), a questo punto so che sicuramente almeno il $+1$ non può crashare. questo vuol dire che tutti gli altri hanno almeno visto 1 messaggio di `propose(v)` con quel valore.

<img src="Immagini/Faults/agree-1.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

#TODO: inserisci foto agree 2

**Terminazione**

>Lemma: l'algoritmo termina in un numero di round nell'ordine di $O(2^n)$

Se una entità non faulty vede una maggioranza su `myvalue` nel round `r` allora l'algoritmo terminerà in `r+1` round

<img src="Immagini/Faults/termination-1.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

Se nessuno vede la maggioranza, vuol dire che nessuno vede $n/2 +1$ valori concordi e può essere che non termini. Se nessuno ha visto la maggioranza di `0` o di `1`, tutti manderanno delle propose con valori casuali. 

l'idea è che la meta più uno dei non faulty debba decidere lo stesso valore.
data una probabilità $p$  che al round successivo ci sia una maggioranza, allora so che:
$$
p \ge \text{ probabilità che tutti scelgano lo stesso valore} \ge 2\frac{1}{2}^n
$$

- la probabilità che ci sia un successo = $p \ge 1/2^n$
- l'insuccesso al round r = $p(1-p)^{r-1}$
  
il numero atteso di round per avere un successo è quindi di $1/p \le 2^n$


>Il protocollo può essere migliorato in modo di avere un numero costante di round aggiungendo una restrizione dove il numero di crash supportati è $n/3$

### Consenso con Byzantine faults

Un byzantine fault è un fallimento molto più difficile da gestire, in quanto un bizantino può simulare un crash, o altro, potrebbero essersi messi d'accordo per remare contro un protocollo (esempio durante un attacco)

> Teorema: non è possibile creare un protocollo deterministico che raggiunge il consenso al massimo di $f\ge n/3$ byzantine failure, anche con un fully connected syncronous graph.

Aumentiamo quindi le restrizioni:
- Grafo completo
- Strong connectivity
- Sistema sincrono
- Tipo di Fallimento: bizzantino
- Inizio Simultaneo
- ID univoci
- Le entità conoscono gli ID dei loro vicini

**Idea**: bisogna vedere se è possibile riutilizzare l'idea che avevamo già avuto.

chi riceve uno zero la prima volta che lo riceve lo manda a tutti gli altri, se non ricevo nulla è un 1. rispetto al caso dei crash qui cosa può andare male:
- nel caso dei crash veniva spedito lo 0 oppure no
- qui può succedere anche di peggio
  - quando un bizzantino lavora potrebbe mandare lo 0 a qualcuno e qualcuno no
  - mandare più volte lo 0
  - far finta di essere qualcun'altro

Abbiamo identificativi univoci, li usiamo mettendoli nei messaggi come mittenti di essi.
Abbiamo un sistema sincrono e uno start simultaneo, quindi nel messaggio aggiungiamo pure il timestamp

Il messaggio qundi diventa `(0, id(x), t)`

- l'identificativo permette di riconoscere il mittente (se conosco gli id dei miei vicini allora so se uno si spaccia per un altro)
- avendo il tempo, i bizzantini non possono mentire sul timestamp
  
**Osservazione**
in questo contesto non possiamo combattere i bizzantini al 100%, alcune cose non possiamo farle, non possiamo sapere se il valore è stato originato da una entità bizzantina o da una non faulty.

Vogliamo che se un bizzantino fa partire uno 0 allora tutti quanti i non faulty devono decidere per quello 0  (non deve far decidere alcuni per 0 altri per 1).

Fondamentalmente il rpotocollo che vogliamo provare a progettare deve fare qualcosa di simile a quello che abbiamo visto prima:
- chi vede uno 0 per la prima volta lo manda a tutti, se questo parta da un non faulty allora devono accettarlo tutti
- se un bizzantino fa partre un wakeup è accettato se tutte le entità non faulty lo ricevono

>Un `wakeup` è l'atto di ricevere(opzionale) e inoltrare per la prima volta uno `0`

Prima di vedere il protocollo vediamo il metodo con cui invieremo i messaggi ai nostri vicini tramite questo protocollo, che soddisfa i due punti visti sopra

#### RegisteredMail

Il meccanismo funziona simile al broadcast con faulty link. ci sono due messaggi che viaggiano se si vuole mandare uno 0.

Il protocollo si spiega per una entità non faulty (fgiustamente i bizzantini potrebbero non seguirlo):

1. per iniziare un wake-up al tempo `t`: `broadcast(init,0, id(x), t)`
2. se ricevo un messaggio `(init, 0, id(x), t)` da `y` al tempo `t+1`, questo viene considerato un messaggio corretto sia nel mittente che nel tempo, lo considero come un inizio di wake-up legittimo, quindi inoltro `(echo, 0, id(x), t)` al tempo `t+1`
3. se ricevo un `(init, 0, id(x), t)` al tempo $t'$ lo si ignora se:
   1. $t' \neq t+1$
   2. il messaggio non è da `y`
   3. o se ho già ricevuto un `(echo, 0, id(x), t'')` con $t'' \neq t+1$
4. se rievo un echo al tempo $t' \ge t+2$ da almeno $f+1$ entità differenti: faccio `broadcast(echo, 0, id(y), t)` al tempo $t'$ (se non è stato fatto prima)
5. se ricevo un echo al tempo $t' \ge t+2$ da almeno $n-f$ entità: accetto il wake-up da `y` al tempo `t` (se non è stato fatto prima)
  
Se un non faulty manda un wake-up è accettato da tutte le entità non faulty al tempo $t+2$

<img src="Immagini/Faults/regmail-1.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

Se c'è una entità non faulty `y` che accetta un init ad un determinato tempo $t'$ allora tutte le entità non faulty accettano lo stesso init al tempo $t'+1$ 

<div style="display: flex; justify-content: space-around; padding-bottom:1.5rem;">
    <img src="Immagini/Faults/regmail-2.png" width="48%" />
    <img src="Immagini/Faults/regmail-3.png" width="48%" />
</div>

<img src="Immagini/Faults/regmail-5.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>


Se un init con un identificativo che non è quello corretto è mandato non è accettato da entità non faulty. se qualcuno cerca di forgare un init (spacciandosi per altri) non verrà accettato.

In tutti i casi anche se è stato forgiato un init da un bizzantino e gli altri bizzantini cercano di propagarlo il protocollo deve comunque accorgersi del problema

<img src="Immagini/Faults/regmail-6.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

### Tell Zero Biz
1. Al tempo 0: ogni entità non faulty con valore iniziale 0 iniziano il RegisteredMail per mandare `(0,id(x),0)`
2. al tempo $2i$ per $1 \le i \le f$


**Termina per costruzione**

**Non triviality**

Se tutte le entità partono con 0, allora tutte le entità non faulty inizieranno con 0.
Assumiamo che le entità inizino con 0

<img src="Immagini/Faults/tell-zero-1.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

Assumiamo che le entità partano con 1

<img src="Immagini/Faults/tell-zero-2.png"  width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

**Agreement (non chiesto all'esame)**

>Lemma: il protocollo TellZero_Byz garantisce l'arrivo al consenso

<img src="Immagini/Faults/tell-zero-3.png"  width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

Se sono tutti partiti con 0 (e non sono faulty) hanno fatto partire un wake-up e questi wake-up sono stati accettati dagli altri

<img src="Immagini/Faults/tell-zero-4.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

<img src="Immagini/Faults/tell-zero-5.png" width="90%" style="display:block;margin:0 auto;padding-bottom:2rem;"/>

Supponiamo che esista un $y \in R$  che era partito con 1 (non fa partire all'inizio al registered mail), se è tra quelli che hanno fatto decidere per 0 allora ci deve essere stata una iterazione in cui lui ha mandato il suo wake-up, vuol dire che in quella iterazione aveva ricevuto abbastanza wake-up per farlo decidere per 0.

<div style="display: flex; justify-content: space-around; padding-bottom:1.5rem;">
    <img src="Immagini/Faults/tell-zero-6.png" width="48%" />
    <img src="Immagini/Faults/tell-zero-7.png" width="48%" />
</div>

> `i = f + 1` è l'ultimo stage

#### Costo

**Messaggi**:
- se ci concentriamo su i non faulty fanno al massimo un wake-up ciascuno ovvero
  - broadcast dell'init, $n-1$ messaggi
  - echo (nel caso peggiore dove tutti seguono il protocollo) $n(n-1)$ messaggi
  - Considerando che sono $n-f$ allora $(n-f)[(n-1)(n+1)]$
- ogni entità faulty il peggio che possono fare è mandare un messaggio ad ogni vicino per ogni unità di tempo
  - $f[2(f+2)(n-1)]$
- Questi messaggi, se inviati al tempo giusto, possono provocare degli echo, pero al massimo una volta ognuno
  - $f[n(n-1)]$

Mettendo tutto assieme
$$
[(n-f)(n+1)+2f(f+2)+nf](n-1) \in O(n^3)
$$

Questi protocolli si possono generalizzare a situazioni meno restrittive:
- Grafi non totalmente connessi
- con valori diversi da `0,1`
